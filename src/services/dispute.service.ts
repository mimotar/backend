...
export async function createDisputeService(data: any) {
    try {
        ...
        const isCreator = order.creator_email === userId || order.user?.id === userId;
        const isReceiver = order.reciever_email === userId;
        
        if (!isCreator && !isReceiver) {
            return { 
                status: 403, 
                message: "Unauthorized: You must be either the sender or receiver of this transaction to create a dispute" 
            };
        }
...
export async function getAllUserDisputes(userId: number){
    try {
        ...
        return { status: 200, data: disputes, message: "Disputes fetched successfully" };
    } catch (error) {
